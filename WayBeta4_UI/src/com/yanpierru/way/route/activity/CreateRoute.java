...
import com.yanpierru.way.PlaceJSONParser;
import com.yanpierru.way.R;
import com.yanpierru.way.asyncTasks.GettingRoute;
...
public class CreateRoute extends Activity {
		...
		mMap.setOnMapLongClickListener(new OnMapLongClickListener() {
			...
			public void onMapLongClick(LatLng point) {
				...
				if(mListMarkers.size()>0){
					...
					alertDialog
							.setMessage("Vous allez réinitialiser votre trajet (effacement des points).\nVoulez vous continuer ?")
							.setCancelable(false)
	...
	private void eraseCurrentRoute(LatLng point){
	...
	// Telecharge des donnees en json depuis une url passee en param
		...
		public void actionCorrection() {
			if (!correctionEnable) {
				...
				getActionBar().setTitle(mRoute.getName()+" [Correction]");
